$(function() {
  ...
  var AppView = Backbone.View.extend({
    ...
    initialize: function() {
      ...
      this.listenTo(UnreadArticleList, 'reset', this.resetUnreadList);
      this.listenTo(ReadArticleList, 'reset', this.resetReadList);
      this.listenTo(ActiveArticle, 'change', this.activeArticleChange);

      // Initialize sources
      // Need to fix google news description - not speech friendly
    ...
    getArticles: function(e) {
      ReadArticleList.reset();
      UnreadArticleList.reset();
      this.disablePlayer();
    ...
    removeArticle: function(article) {
      ...
      this.resetUnreadList();

      if (UnreadArticleList.length === 0) {
        this.disablePlayer();
      }
      else {
        UnreadArticleList.each(function(model, index, list) {
          App.addArticle(model);
        });
      }
    },

    resetUnreadList: function() {
      // Clear the Unread Articles view
      this.$("#queued-content").html("");
    },

    resetReadList: function() {
      // Clear the Unread Articles view
      this.$("#played-content").html("");
    },

    activeArticleChange: function(){
      console.log('article changed');
      var view = new ArticleView({model: ActiveArticle});
      this.$('#active-content').html(view.render().el);
    },
    ...
    playActive: function() {
      ...
      if (!ActiveArticle.id) {
        ActiveArticle.set(UnreadArticleList.shift().toJSON());
      }
